...
# ╔═╡ c1ac7d74-f24b-46b2-85ba-5b29489a8c67
begin
	using NPZ
	using Plots
	using Images
...
# ╔═╡ a8276560-8352-11ed-1359-014cf485f18d
begin
	data = npzread("tiny_nerf_data.npz")
	images = data["images"]
	poses = data["poses"]
	focal = data["focal"]
	println("Images shape: $(size(images))")
	println("Poses shape: $(size(poses))")
	...
	println("Pose")
	println(testpose)
	testimg_reshaped = reshape(testimg, (3,100,100))
	colorview(RGB, testimg_reshaped)
end

# ╔═╡ 577d85db-7e2a-4d62-b14c-500501a106a9
begin
	x_dims = size(poses, 1)
	poses_reshape = poses[:, 1:3, 1:3]
	dirs = zeros(x_dims, 3)
	for i in 1:x_dims
		temp = [[0,0,0] [0,0,0] [-1,-1,-1]] .* poses_reshape[i, :, :]
		global dirs[i,:] = temp[:,3]
	end
	origins = poses[:, 1:3, 4]
	quiver(origins[:, 1], origins[:, 2], origins[:, 3], quiver=(dirs[:, 1], dirs[:, 2], dirs[:, 3]), arrow=arrow(0.01, 0.01))
end

# ╔═╡ 56eb473c-4ed1-4273-848a-6745158fa196
begin
	...
	function get_rays(height, width, focal_length, c2w)
		# find origin and direction of rays through every pixel and camera origin
		i, j = meshgrid(0:width-1, 0:height-1)
		directions = cat(cat((i.-(width*.5)) / focal_length, ((j .- (height*.5)) .* -1 ) / focal_length, dims=3), fill(-1.0, (100,100)); dims=3)
		# display(reshape(directions,(100,3,1,100)))
		return 0, 0
	end
	images_training = images[1:n_training, :, :, :]
	height, width = size(images_training)[2:3]
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "e91564d6bb98c1f56ace7e73192fd543d483ee6e"
